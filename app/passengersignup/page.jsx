  ...
  return (
    <div className='h-[100vh] overflow-hidden max-sm:pl-0 max-sm:pr-0 bg-gradient-to-r from-[#0ab9cf] to-[#3581d8] flex pl-10 pr-14 justify-center items-center navshad'>
      ...
      <div className='flex h-[90vh] flex-col'>
        ...
        <ToastContainer />
        <form onSubmit={handleSubmit(onSubmit)} className="w-[450px] h-[80vh] example overflow-auto max-sm:w-[300px] px-7 py-10 max-sm:px-2 max-sm:py-4 bg-[#ffffff2d] max-sm:text-sm rounded-xl lsshad flex flex-col">
          <div className="mb-4">
          ...
          <div className="flex items-center mt-7">
            <Link href={'/driversignup'}>
              <button className="bg-[#3581d8] max-sm:text-sm smooth hover:text-[#000000be] hover:bg-[#ffffff] btnshad text-white font-bold py-2 px-5 rounded-md focus:outline-none focus:shadow-outline" type="button">Sign up as a DRIVER</button>
            </Link>
          </div>

          <div className="flex items-center gap-2 mt-7 max-sm:flex-col">
            <span className='font-semibold text-[17px] max-sm:text-[14px] text-[#000000d2]'>I already have an account as a passenger</span>
            <Link href={'/'}>
              <button className="text-[18px] max-sm:text-[15px] smooth befaft relative text-white font-bold rounded-md focus:outline-none" type="button">
                Log in
              </button>
            </Link>
          </div>
