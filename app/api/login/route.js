    ...
    if(useraccount && payload.usertype=="user"){
	   ...
	   var originalText = bytes.toString(CryptoJS.enc.Utf8);
		...
		if(payload.email== useraccount.email && payload.password==originalText){
			var token = jwt.sign({email:useraccount.email,username:useraccount.username,password:useraccount.password,usertype:useraccount.usertype}, '@deekshigowda');
        ...
        console.log();
         if(payload.email== driveraccount.email && payload.password==originalText){
             var token = jwt.sign({email:driveraccount.email,username:driveraccount.username,password:driveraccount.password}, '@deekshigowda');
